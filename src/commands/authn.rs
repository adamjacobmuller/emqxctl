    ...
    match cmd {
        ...
        AuthnCommand::Delete { id } => {
            super::handle_delete(
                ...
                fmt,
                &format!("/authentication/{}", id),
                &format!("Authenticator '{}' deleted", id),
            )
            .await?;
        }
        ...
        AuthnCommand::Users { id, command } => {
            ...
            match command {
                ...
                AuthnUsersCommand::Get { userid } => {
                    super::handle_get(client, fmt, &format!("{}/{}", base, userid), USER_COLUMNS)
                        .await?;
                }
        ...
        AuthnCommand::Import { id, file } => {
            client
                .upload(&format!("/authentication/{}/import_users", id), file)
                .await?;
