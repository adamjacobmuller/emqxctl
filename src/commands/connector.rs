...
pub async fn execute(client: &EmqxClient, fmt: &OutputFormatter, cmd: &ConnectorCommand) -> Result<()> {
    match cmd {
        ConnectorCommand::List { connector_type } => {
            let value = if let Some(t) = connector_type {
                client.get_with_query("/connectors", &[("type", t.as_str())]).await?
            } else {
        ...
        ConnectorCommand::Create { file } => {
            super::handle_create_or_update(client, fmt, Method::POST, "/connectors", file, "Connector created").await?;
        }
        ConnectorCommand::Update { id, file } => {
            super::handle_create_or_update(client, fmt, Method::PUT, &format!("/connectors/{}", id), file, "Connector updated").await?;
        }
        ConnectorCommand::Delete { id } => {
            super::handle_delete(client, fmt, &format!("/connectors/{}", id), &format!("Connector '{}' deleted", id)).await?;
        }
        ConnectorCommand::Test { id } => {
            let result = client.post(&format!("/connectors/{}/test", id), &serde_json::json!({})).await?;
            fmt.print_value(&result);
        }
        ConnectorCommand::Start { id } => {
            client.post(&format!("/connectors/{}/start", id), &serde_json::json!({})).await?;
            fmt.print_success(&format!("Connector '{}' started", id));
        }
        ConnectorCommand::Stop { id } => {
            client.post(&format!("/connectors/{}/stop", id), &serde_json::json!({})).await?;
            fmt.print_success(&format!("Connector '{}' stopped", id));
        }
        ConnectorCommand::Restart { id } => {
            client.post(&format!("/connectors/{}/restart", id), &serde_json::json!({})).await?;
