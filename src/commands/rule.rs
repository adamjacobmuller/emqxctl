...
pub enum RuleCommand {
    ...
    ResetMetrics { id: String },
}

const LIST_COLUMNS: &[Column] = &[
    Column { header: "ID", json_path: "id", max_width: Some(20) },
    Column { header: "NAME", json_path: "name", max_width: Some(20) },
    Column { header: "ENABLED", json_path: "enable", max_width: None },
    Column { header: "SQL", json_path: "sql", max_width: Some(50) },
    Column { header: "ACTIONS", json_path: "actions", max_width: Some(30) },
];
...
const DETAIL_COLUMNS: &[Column] = &[
    ...
    Column { header: "ENABLED", json_path: "enable", max_width: None },
    Column { header: "SQL", json_path: "sql", max_width: None },
    Column { header: "ACTIONS", json_path: "actions", max_width: None },
...
pub async fn execute(client: &EmqxClient, fmt: &OutputFormatter, cmd: &RuleCommand) -> Result<()> {
    match cmd {
        ...
        RuleCommand::Create { file } => {
            super::handle_create_or_update(client, fmt, Method::POST, "/rules", file, "Rule created").await?;
        }
        RuleCommand::Update { id, file } => {
            super::handle_create_or_update(client, fmt, Method::PUT, &format!("/rules/{}", id), file, "Rule updated").await?;
        }
        RuleCommand::Delete { id } => {
            super::handle_delete(client, fmt, &format!("/rules/{}", id), &format!("Rule '{}' deleted", id)).await?;
        }
        ...
        RuleCommand::ResetMetrics { id } => {
            client.put(&format!("/rules/{}/metrics/reset", id), &serde_json::Value::Null).await?;
