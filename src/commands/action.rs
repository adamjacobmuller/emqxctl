...
const LIST_COLUMNS: &[Column] = &[
    ...
    Column { header: "CONNECTOR", json_path: "connector", max_width: Some(20) },
];
...
const DETAIL_COLUMNS: &[Column] = &[
    ...
    Column { header: "STATUS", json_path: "status", max_width: None },
    Column { header: "CONNECTOR", json_path: "connector", max_width: None },
    Column { header: "DESCRIPTION", json_path: "description", max_width: None },
];

pub async fn execute(client: &EmqxClient, fmt: &OutputFormatter, cmd: &ActionCommand) -> Result<()> {
    match cmd {
        ActionCommand::List => {
        ...
        ActionCommand::Create { file } => {
            super::handle_create_or_update(client, fmt, Method::POST, "/actions", file, "Action created").await?;
        }
        ActionCommand::Update { id, file } => {
            super::handle_create_or_update(client, fmt, Method::PUT, &format!("/actions/{}", id), file, "Action updated").await?;
        }
        ActionCommand::Delete { id } => {
            super::handle_delete(client, fmt, &format!("/actions/{}", id), &format!("Action '{}' deleted", id)).await?;
        }
        ...
        ActionCommand::Start { id } => {
            client.post(&format!("/actions/{}/start", id), &serde_json::json!({})).await?;
            fmt.print_success(&format!("Action '{}' started", id));
        }
        ActionCommand::Stop { id } => {
            client.post(&format!("/actions/{}/stop", id), &serde_json::json!({})).await?;
