...
const LIST_COLUMNS: &[Column] = &[
    ...
    Column { header: "MESSAGE", json_path: "message", max_width: Some(40) },
    Column { header: "ACTIVATE AT", json_path: "activate_at", max_width: None },
    Column { header: "DEACTIVATE AT", json_path: "deactivate_at", max_width: None },
];

pub async fn execute(client: &EmqxClient, fmt: &OutputFormatter, cmd: &AlarmCommand) -> Result<()> {
    match cmd {
        AlarmCommand::List { pagination, activated, deactivated } => {
            ...
            super::handle_paginated_list(client, fmt, "/alarms", &query, pagination, LIST_COLUMNS, None).await?;
        }
        AlarmCommand::Clear => {
