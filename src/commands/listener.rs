    ...
    match cmd {
        ...
        ListenerCommand::Delete { id } => {
            super::handle_delete(
                ...
                fmt,
                &format!("/listeners/{}", id),
                &format!("Listener '{}' deleted", id),
            )
            .await?;
        }
        ListenerCommand::Start { id } => {
            client
                .post(&format!("/listeners/{}/start", id), &serde_json::json!({}))
                .await?;
            fmt.print_success(&format!("Listener '{}' started", id));
        }
        ListenerCommand::Stop { id } => {
            client
                .post(&format!("/listeners/{}/stop", id), &serde_json::json!({}))
                .await?;
