...
pub async fn execute(client: &EmqxClient, fmt: &OutputFormatter, cmd: &TopicCommand) -> Result<()> {
    match cmd {
        TopicCommand::List { pagination, topic } => {
            ...
            if let Some(ref t) = topic { query.push(("topic", t.clone())); }
            super::handle_paginated_list(client, fmt, "/topics", &query, pagination, LIST_COLUMNS, None).await?;
        }
        TopicCommand::Get { topic } => {
            let value = client.get(&format!("/topics/{}", urlencoding_simple(topic))).await?;
...
fn urlencoding_simple(s: &str) -> String {
    s.replace('/', "%2F").replace('#', "%23").replace('+', "%2B")
}
